...
func main() {
	...
	flagset.Var(&queryParams, "query-param", "Name of the HTTP query or form parameter that contains the label value. Cannot be combined with -header-name or -label-value. Repeat once per -label; labels and parameters are paired by position. If no value source is configured, each label name is used as its query parameter.")
	flagset.Var(&headerNames, "header-name", "Name of the HTTP header that contains the label value. Cannot be combined with -query-param or -label-value. Repeat once per -label; labels and headers are paired by position.")
	...
	flagset.Var(&labels, "label", "The label name to enforce in proxied requests and responses. Repeat to enforce multiple labels.")
	flagset.Var(&labelValues, "label-value", "A fixed label value to enforce in proxied requests and responses. Cannot be combined with -query-param or -header-name and only supports one -label. Repeat to enforce the union of values for that label.")
